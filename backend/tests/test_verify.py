...
from javasmell.refactor.verify import Verdict, check, count_errors, parses_cleanly
...
def test_a_file_that_never_compiled_is_judged_on_whether_it_got_worse():
    ...
    assert count_errors(JAVAC, NEEDS_IMPORT, "T.java") > 0

    result = check(JAVAC, NEEDS_IMPORT, NEEDS_IMPORT, "T.java")
    assert result.verdict is Verdict.NO_NEW_ERRORS
    assert result.verdict.passed
    assert result.errors_before == result.errors_after


@pytest.mark.skipif(JAVAC is None, reason="javac not on PATH")
def test_adding_an_error_to_an_already_failing_file_is_caught():
    worse = NEEDS_IMPORT.replace(b"}\n", b"    void m() { alsoUndefined(); }\n}\n")
