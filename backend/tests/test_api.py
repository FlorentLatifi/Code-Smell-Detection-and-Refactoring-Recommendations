...
def test_the_summary_says_how_many_files_did_not_parse(client, tmp_path):
    ...
    Without this count a project where half the files fail shows fewer smells
    and looks like better code (VD-91).
    """
    (tmp_path / "workspace" / "src" / "Broken.java").write_text(
        "public class Broken {\n  void f( {\n", encoding="utf-8"
    )
    summary = client.post("/analyze", json={"path": "src"}).json()["summary"]

    assert summary["unparsed"] == 1
    assert summary["files"] > summary["unparsed"]


def test_a_clean_project_reports_nothing_unparsed(client):
