...
shtatorit; ajo u rimat dy herë pas kësaj (VD-137 dhe VD-138), dhe numrat që raporton
punimi janë te VD-138.
...
- **Kohëzgjatja.** Matja u ndërpre disa herë nga fjetja e makinës dhe vazhdoi nga
  ...
  më; te VD-137 ajo u bë kumulative, por mospërmendja mbeti.
- **Dy përmbysjet** (Extract Method në Hive dhe në Eclipse JDT) nuk u shpjeguan
  ...
  dhënat, dhe VD-137 e ruan edhe shkakun.
...
**Mbetur i hapur, i zgjidhur te VD-137.** `refactoring_sites.csv` dhe
...
### VD-137: Rezultatet mbajnë shtigje relative, dhe përmbysjet mbajnë shkakun
...
dhe numrat përfundimtarë janë te VD-138; kjo rimatje nuk është ajo që raporton punimi.
...
### VD-138: Një javac që dështon pa emërtuar gabim nuk është kompilim i pastër

**Konteksti.** Rimatja e VD-137 dha të njëjtat totale si matja e mëparshme, por
krahasimi rresht për rresht nxori 21 verdikte kompilimi të ndryshuara, të cilat
ngarkesa e makinës nuk i shpjegonte. Shkaku ishte te leximi i daljes së `javac`-ut:
`error_messages` kthente bashkësinë e gabimeve që mbanin shenjën `: error:`, dhe kur
`javac` dilte me kod jo-zero pa asnjë rresht të tillë, bashkësia dilte bosh. Një
bashkësi bosh lexohej «pa gabime», pra verdikti më i favorshëm. Pesë skedarë të mëdhenj
të ekzekutimit të 23 shtatorit dolën kështu, dhe 19 rishkrime u regjistruan si «kompilon»
mbi skedarë që nuk kompilojnë fare.

Një dështim i tillë nuk është veti e kodit: është `javac` që nuk shkroi dot një klasë, që
nuk gjeti një skedar, ose një VM që nuk u nis. Rreshtat e tij nisin me `error:`, pa
emrin e skedarit, dhe shenja qëllimisht nuk i kap.

**Vendimi.**
- `verify.errors_in` kthen gabimet e shënuara kur ka të tilla, bashkësi bosh vetëm kur
  `javac` del me kod 0, dhe `None` kur dështon pa emërtuar asnjë gabim mbi burimin.
  `None` do të thotë «javac u pyet por nuk i besohet», dhe verdikti bie te më i dobëti,
  «u parsua», njësoj si te kufiri kohor.
- `verify_with_project.py` e lexon daljen përmes së njëjtës funksion, ndaj të dy matjet
  me kompilator e trajtojnë dështimin njësoj.
- `provenance.javac_version` e regjistron versionin e kompilatorit në çdo skedar
  rezultati që mbështetet te `javac`, sepse «kompilon» është pohim për një kompilator
  po aq sa për kodin.
- Punimi i numëron veçmas rishkrimet pa verdikt (`_unanswered` te 5.4), dhe fjalia
  për kompilimet e pakontrolluara nuk ia atribuon më të gjitha kufirit kohor
  (`_unchecked_sentence`).

**Matja.** Pas ndreqjes, mbi tërë korpusin: 3 459 rishkrime pa gabim të ri, 12 që
kompilojnë, 58 me gabim të ri dhe 3 pa verdikt (ishin 3 460 dhe 2). Mbi mostrën e 60
skedarëve të verifikimit brenda projektit: 4 rishkrime kompilojnë të izoluara dhe 197
nuk shtojnë gabim (ishin 5 dhe 196); brenda projektit 66 kompilojnë, 114 nuk shtojnë
gabim, 2 shtojnë dhe 19 mbeten të pakontrolluara (ishin 110 dhe 23). Të dy skedarët e
rezultateve mbajnë `javac 21`.

**Çfarë nuk u verifikua.** Pesë skedarët që e shfaqën defektin nuk u analizuan një nga
një për të gjetur pse `javac` dështoi pa mesazh; ndreqja nuk varet nga shkaku, sepse çdo
dështim i tillë tani lexohet si mungesë verdikti.

---

### VD-139: Kufiri kohor i javac-ut kufizon vetë kompilatorin

**Konteksti.** Në Windows, `javac` që gjendet te PATH është zakonisht një ndërmjetës
nën `Common Files/Oracle/Java/javapath`, që e nis kompilatorin e vërtetë si proces bir.
Kur skadonte koha, `subprocess.run` vriste ndërmjetësin dhe e linte kompilatorin të
punonte. Për më tepër, me dalje përmes tubave, `subprocess.run` pret që të mbyllet
edhe skaji i shkrimit të tubit, të cilin procesi bir e mbante hapur. Rezultati: një kufi
i deklaruar prej 180 sekondash zgjati 50 minuta mbi një skedar të JDK-së.

**Vendimi.**
- `verify.javac_command` e merr kompilatorin nga `JAVA_HOME/bin` kur ekziston, dhe vetëm
  përndryshe nga PATH, që procesi që vritet të jetë ai që punon.
- `verify.run_javac` e shkruan daljen në një skedar në dosjen e përkohshme, jo në tub,
  ndaj thirrja kthehet sapo skadon koha.
- Argumentet mbeten një listë e fiksuar, me kufi kohe dhe në dosje të përkohshme, siç e
  kërkon `ENGINEERING.md` §6.

**Pasojat.** Kohëzgjatja e matjes brenda projektit, që tani mblidhet nëpër rinisje
(VD-137), është kufi i sipërm dhe punimi vazhdon të mos e citojë.

**Shënim për numrat.** Kjo punë u krye lokalisht nën numrat VD-135, VD-136 dhe VD-137,
