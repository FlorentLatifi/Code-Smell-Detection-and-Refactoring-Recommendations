...
# failed without naming an error: both go through `verify.errors_in` (VD-138).
NEWLINE = chr(10)
...
def new_errors(before: set[str] | None, after: set[str] | None) -> str:
    ...
    two and nothing recorded why (VD-137).
    """
    if before is None or after is None:
        return ""
...
def save_progress(path: Path, done: Rows, partial: Rows, seconds: float) -> None:
    ...
    coincided with a save (VD-137). The work lives in the rows, so a second
